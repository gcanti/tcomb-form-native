...
function datepicker(locals) {
   ...
   */
  var formattedValue = locals.value?String(locals.value):locals.placeholder;
  var background = TouchableNativeFeedback.SelectableBackground(); // eslint-disable-line new-cap
  var dialogMode = "default";
  var formattedDateValue = locals.value?locals.value.toDateString():locals.placeholder;
  var formattedTimeValue = locals.value?locals.value.toTimeString():locals.placeholder;
  if (locals.config) {
    ...
    if (locals.config.dateFormat && locals.value) {
      formattedDateValue = locals.config.dateFormat(locals.value);
    }
    if (locals.config.timeFormat && locals.value) {
  ...
  ) : <Text style={dateValueStyle}>{locals.placeholder}</Text>;
